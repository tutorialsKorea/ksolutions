...
namespace CodeHelperManager
{
    ...
    public class RepositoryItemSimulation : RepositoryItemPopupContainerEdit
    {
        static RepositoryItemSimulation()
        {
            Register();
        }


        public RepositoryItemSimulation()
        {
            base.Buttons.Clear();
        ...
        internal const string EditorName = "acSimulation";

        public static void Register()
        {

            EditorRegistrationInfo.Default.Editors.Add(new EditorClassInfo(EditorName, typeof(acSimulation),
                typeof(RepositoryItemSimulation), typeof(DevExpress.XtraEditors.ViewInfo.PopupContainerEditViewInfo),
    ...
    public class acSimulation : acUserPopupContainerEdit, ControlManager.IBaseEditControl
    {
        ...
        static acSimulation()
        {
            RepositoryItemSimulation.Register();
        }
        ...
        public acSimulation()
        {
            ...
            {
                //기본컬럼
                acSimulation.SetPopupGridView(acGridView1);
            ...
            this.GotFocus += new EventHandler(acSimulation_GotFocus);
            this.LostFocus += new EventHandler(acSimulation_LostFocus);
        }

        void acSimulation_LostFocus(object sender, EventArgs e)
        {
        ...
        void acSimulation_GotFocus(object sender, EventArgs e)
        {
            if (acInfo.SysConfig.GetSysConfigByMemory("FOCUS_EDIT_ENABLED").toBoolean())
            {
        ...
        public static void SetPopupGridView(acGridView view)
        {
            ...
            //기본컬럼
            view.AddTextEdit("SMLT_ID", "시뮬레이션 ID", "GXBNS93J", true , DevExpress.Utils.HorzAlignment.Center, false, true, false, acGridView.emTextEditMask.NONE);

            view.AddLookUpEdit("BOP_REF_TYPE", "BOP 참조형태", "PUYV3ONT", true , DevExpress.Utils.HorzAlignment.Center, false, true, false, "S083");

            view.AddTextEdit("BOP_REF_CODE", "BOP 참조코드", "ZJ2GYZDX", true , DevExpress.Utils.HorzAlignment.Center, false, true, false, acGridView.emTextEditMask.NONE);
        ...
        void acGridView1_OnInitLayout(object sender)
        {
            ...
            view.LoadUserConfig("SYS04B_M0A", "acSimulation", acInfo.DefaultConfigUser);
        ...
        public override string EditorTypeName
        {
            get { return RepositoryItemSimulation.EditorName; }
        }



        [DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
        public new RepositoryItemSimulation Properties
        {
            get
            {

                return base.Properties as RepositoryItemSimulation;
        ...
        protected override void OnKeyDown(System.Windows.Forms.KeyEventArgs e)
        {
            ...
            if (this._isReadyOnly == false)
            {
                ...
                {
                    this.Execute(emMethodType.FIND);
                }
        ...
        void Execute(emMethodType method)
        {


            switch (method)
            {
                case emMethodType.FIND:


                    acSimulationForm frmFind = new acSimulationForm();
                    ...
                    if (frmFind.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        if (!this._Value.EqualsEx(result.Rows[0]["SMLT_ID"]))
                        {
                        ...
                        this._Value = result.Rows[0]["SMLT_ID"];



                        if (this.EditValue.EqualsEx(result.Rows[0].GetStringByMaskScript(acInfo.SysConfig.GetSysConfigByMemory("CTRL_SMLT_SHOW_COLUMN"))))
                        {
                        ...
                        this.EditValue = result.Rows[0].GetStringByMaskScript(acInfo.SysConfig.GetSysConfigByMemory("CTRL_SMLT_SHOW_COLUMN"));
        ...
        public static DataRow GetDataRow(object code)
        {
            ...
            paramTable.Columns.Add("SMLT_ID", typeof(String)); //부품코드

            DataRow paramRow = paramTable.NewRow();
            paramRow["PLT_CODE"] = acInfo.PLT_CODE;
            paramRow["SMLT_ID"] = code;
            paramTable.Rows.Add(paramRow);
            DataSet paramSet = new DataSet();
            paramSet.Tables.Add(paramTable);

            DataTable data = BizRun.QBizRun.ExecuteService(GetClassName(), "CONTROL_SIMULATION_SEARCH", paramSet, "RQSTDT", "RSLTDT").Tables[1];
        ...
        private bool SetCode(object code)
        {
            ...
            if (acChecker.isNull(code) == false)
            {
                ...
                DataTable result = BizRun.QBizRun.ExecuteService(this, "CONTROL_SIMULATION_SEARCH", paramSet, "RQSTDT", "RSLTDT").Tables[1];
