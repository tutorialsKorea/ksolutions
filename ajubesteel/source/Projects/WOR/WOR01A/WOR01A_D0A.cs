...
namespace WOR
{
    public sealed partial class WOR01A_D0A : BaseMenuDialog
    {
        ...
        public WOR01A_D0A(acGridView linkView, object linkData, string empCode, bool isPopup = false)
        {
            InitializeComponent();
        ...
        private void acLayoutControl1_OnValueChanged(object sender, IBaseEditControl info, object newValue)
        {
            ...
            switch (info.ColumnName)
            {
                case "WORK_CODE":
                    ...
                    (layout.GetEditor("SCOMMNET_SEL") as acLookupEdit).SetData("CAUSE_NAME", "CAUSE_CODE", resultSet.Tables["RSLTDT"]);
            ...
            try
            {
                ...
                if (workRows.Length == 1)
                {
                    ...
                    if (workRows[0]["IS_YESTERDAY"].ToString() == "1")
                    {
                        ...
                        /*
                         * 10시 이전 옵션 True 이며
                         * 작업날 이후가 휴일,주말, 연차(오전반차 포함)이면 10시 이전 옵션 제외
                         * 
                         * 1. 입력일(완료일시) 이후의 휴일 불러오기(공휴일, MES 사용자 지정휴일)
                         * 2. 오늘과 입력일(완료일시) 휴일만 존재하는지 체크 (평일이 존재할시 기존 로직 적용)
                         */
                        if(Common.Instance.IsOnlyHoliday(startDate, nowdatetime) == false)
                        {
                            if (compDays == 1)
                            {
                            ...
                            if (isYesterday)
                            {
                                acAlert.Show(this, "오전 9시까지만 등록 가능합니다.", acAlertForm.enmType.Warning);
                                return;
                            }
                        }
                    }
                ...
                if (paramRow["WORK_CODE"].ToString() == "W08")
                {
                    DataTable validTable = new DataTable("RQSTDT");
                    validTable.Columns.Add("PLT_CODE", typeof(string));
                    validTable.Columns.Add("WORK_CODE", typeof(string));
                    validTable.Columns.Add("EMP_CODE", typeof(string));
                    validTable.Columns.Add("REQ_START_DATE", typeof(string));

                    DataRow validRow = validTable.NewRow();
                    validRow["PLT_CODE"] = acInfo.PLT_CODE;
                    validRow["WORK_CODE"] = paramRow["WORK_CODE"];
                    validRow["EMP_CODE"] = _EmpCode;
                    validRow["REQ_START_DATE"] = paramRow["REQ_START_DATE"].toDateString("yyyyMMdd");

                    validTable.Rows.Add(validRow);

                    DataSet validSet = new DataSet();
                    validSet.Tables.Add(validTable);

                    DataSet validResultSet = BizRun.QBizRun.ExecuteService(this, "WOR01A_SER11", validSet, "RQSTDT", "RSLTDT");

                    if (validResultSet.Tables["RSLTDT"].Rows.Count > 0)
                    {
                        if (acMessageBox.Show(this, "해당일에 잔업이 등록되어 있습니다. 추가로 등록하시겠습니까?", "", false, acMessageBox.emMessageBoxType.YESNO) == DialogResult.No)
                        {
                            return;
                        }
                    }
                ...
                "WOR01A_INS", paramSet, "RQSTDT,RQSTDT2", "RSLTDT",
                QuickSave,
                QuickException);
        ...
        bool TimeValidCheck(acLayoutControl layout, string inputType)
        {
            ...
            if (inputType == "F")
            {
                ...
                if (layout.GetEditor("S_MIN_TEXT").Value.ToString() == ""
                    || layout.GetEditor("E_MIN_TEXT").Value.ToString() == "")
                {
                    string day1 = layout.GetEditor("S_REQ_DATE").Value.ToString();
                    string hour1 = layout.GetEditor("S_HOUR").Value.ToString().PadLeft(2, '0');
                    string day2 = layout.GetEditor("E_REQ_DATE").Value.ToString();
        ...
        void SetItemSize(acLayoutControlItem item, int iMinus = 0)
        {
            ...
            int width = item.TextSize.Width + 130 + margin - iMinus;
