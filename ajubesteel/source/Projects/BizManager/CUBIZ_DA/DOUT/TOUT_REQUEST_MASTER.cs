...
namespace DOUT
{
    public class TOUT_REQUEST_MASTER
    {

        public static void TOUT_REQUEST_MASTER_INS(DataTable dtParam, BizExecute.BizExecute bizExecute)
        {
            try
            {
                ...
                if (dtParam.Rows.Count > 0)
                {
                    ...
                    sbQuery.Append(" INSERT INTO TOUT_REQUEST_MASTER");
                    sbQuery.Append(" (								");
        ...
        public static void TOUT_REQUEST_MASTER_UPD(DataTable dtParam, BizExecute.BizExecute bizExecute)
        {
            try
            {
                DataSet dsResult = new DataSet();
                ...
                if (dtParam.Rows.Count > 0)
                {
                    ...
                    sbQuery.Append(" UPDATE TOUT_REQUEST_MASTER	   ");
    ...
    public class TOUT_REQUEST_MASTER_QUERY
    {
        public static DataTable TOUT_REQUEST_MASTER_QUERY2(DataTable dtParam, BizExecute.BizExecute bizExecute)
        {
            try
            {
                DataSet dsResult = new DataSet();
                ...
                if (dtParam.Rows.Count > 0)
                {
                    ...
                    sbQuery.Append(" FROM TOUT_REQUEST_MASTER RM	 ");
                    ...
                    foreach (DataRow row in dtParam.Rows)
                    {
                        ...
                        sbWhere.Append(UTIL.GetWhere(row, "@REQ_STAT", " RM.REQ_STAT = @REQ_STAT AND (SELECT COUNT(*) FROM TOUT_REQUEST WHERE PLT_CODE = RM.PLT_CODE AND REQUEST_NO = RM.REQUEST_NO AND REQ_STAT = @REQ_STAT) > 0 "));
