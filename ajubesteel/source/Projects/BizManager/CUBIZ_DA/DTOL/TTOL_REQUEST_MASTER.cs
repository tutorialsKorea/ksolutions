...
namespace DTOL
{
    public class TTOL_REQUEST_MASTER
    {
    }

    public class TTOL_REQUEST_MASTER_QUERY
    {
        public static DataTable TTOL_REQUEST_MASTER_QUERY2(DataTable dtParam, BizExecute.BizExecute bizExecute)
        {
            try
            {
                ...
                if (dtParam.Rows.Count > 0)
                {
                    ...
                    sbQuery.Append(" FROM TTOL_REQUEST_MASTER RM	 ");
                    ...
                    foreach (DataRow row in dtParam.Rows)
                    {
                        ...
                        sbWhere.Append(UTIL.GetWhere(row, "@REQ_STAT", " RM.REQ_STAT = @REQ_STAT AND (SELECT COUNT(*) FROM TTOL_REQUEST WHERE PLT_CODE = RM.PLT_CODE AND REQUEST_NO = RM.REQUEST_NO AND REQ_STAT = @REQ_STAT) > 0 "));
