...
{
    public sealed partial class acAttachFileControl2 : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private bool _IsPurAttach = false;
        ...
        [EditorBrowsable(EditorBrowsableState.Never)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public bool IsPurAttach
        {
            get { return _IsPurAttach; }
            set
            {
                _IsPurAttach = value;
        ...
        public acAttachFileControl2()
        {
            ...
            if (ControlManager.acInfo.IsRunTime == true)
            {
                ...
                FileGridView.AddHidden("FILE_ID", typeof(String));
        ...
        void FileGridControl_Paint(object sender, PaintEventArgs e)
        {
            if (acInfo.IsRunTime == true)
            {
                ...
                if (_IsPurAttach)
                {
                    string msg = "발주처가 여려개일 경우 확인 및 취소 탭에서 등록가능합니다.";
        ...
        void FileGridView_ShowGridMenuEx(object sender, PopupMenuShowingEventArgs e)
        {
            ...
            DataRow focus = view.GetFocusedDataRow();

          
            if (focus != null)
            {
                if (!focus.Table.Columns.Contains("PLT_CODE")) { focus.Table.Columns.Add("PLT_CODE", typeof(String), ""); } //21.06.21(kdpark)

                if (focus["PLT_CODE"].ToString() == "")
                {
                    acBarSubItem1.SetVisibility(true, DevExpress.XtraBars.BarItemVisibility.Never);
        ...
        void FileTransferGridControl_DragDrop(object sender, DragEventArgs e)
        {
            ...
            try
            {
                ...
                if (data != null)
                {
                    ...
                    DataTable dt2 = FileGridView.NewTable();

                    UploadAddFileList2(new object[] { dt, fileNames, dt2});

                    if (!_dtFileName.Columns.Contains("FILE_FULL_NAME"))
                    {
                        _dtFileName.Columns.Add("FILE_FULL_NAME", typeof(String));
                    }
                    if (!_dtFileName.Columns.Contains("FILE_NAME"))
                    {
                        _dtFileName.Columns.Add("FILE_NAME", typeof(String));
                    }

                    int a = fileNames.Length;
                    foreach (string _strFileName in fileNames)
                    {
                        DataRow row = _dtFileName.NewRow();
                        FileInfo fInfo = new FileInfo(_strFileName);
                        row["FILE_FULL_NAME"] = _strFileName;
                        row["FILE_NAME"] = fInfo.Name;
        ...
        void FtpUploadSuccess(acFTP ftp, FtpFile file)
        {
            ...
            try
            {
                ...
                if (FileTransferGridView.RowCount == 0)
                {
                    acTabControl1.SelectedTabPage = acTabPage1;

                    _dtFileName.Rows.Clear();
        ...
        private DataTable _dtFileName = new DataTable("FileName");
        private void btnUpload_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            try
            {
                ...
                if (dlg.ShowDialog() == DialogResult.OK)
                {
                    ...
                    UploadAddFileList2(new object[] { dt, dlg.FileNames, dt2 });
                    ...
                    int a = dlg.FileNames.Length;
                    foreach (string _strFileName in dlg.FileNames)
                    {
                        DataRow row = _dtFileName.NewRow();
                        FileInfo fInfo = new FileInfo(_strFileName);
                        row["FILE_FULL_NAME"] = _strFileName;
        ...
        {
            if (_dtFileName.Rows.Count != 0)
            {
                BizManager.QThread qt = new BizManager.QThread(this, BizManager.QThread.emExecuteType.PROCESS);

                DataTable dt = FileTransferGridView.NewTable();

                string[] FileNames = new string[_dtFileName.Rows.Count];

                for (int i = 0; i < _dtFileName.Rows.Count; i++)
                {
                    FileNames[i] = _dtFileName.Rows[i]["FILE_FULL_NAME"].ToString();
                }
        ...
        public void UploadFileWaitThread(int waitSec)
        {
            if (_dtFileName.Rows.Count != 0)
            {
                BizManager.QThread qt = new BizManager.QThread(this, BizManager.QThread.emExecuteType.PROCESS);
                ...
                while (qt.Thread.IsAlive && cnt < waitSec)
                {
        ...
        void UploadAddFileList2(object args)
        {
            ...
            DataTable nTable2 = data[2] as DataTable;
            ...
            foreach (string fileName in fileNames)
            {
                ...
                DataRow newRow2 = nTable2.NewRow();

                newRow2["SEL"] = "0";

                newRow2["FILE_ID"] = FileGridView.RowCount + 1;

                newRow2["ACC_LEVEL"] = "I";

                newRow2["FILE_NAME"] = fInfo.Name;

                newRow2["FILE_SIZE"] = fInfo.Length;

                nTable2.Rows.Add(newRow2);

                FileGridView.AddRow(newRow2);
        ...
        private void btnDelete_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            try
            {
                ...
                DataTable _delparamTable2 = new DataTable("RQSTDT2");

                if (!_delparamTable2.Columns.Contains("DEL_EMP")) _delparamTable2.Columns.Add("DEL_EMP", typeof(String)); //
                if (!_delparamTable2.Columns.Contains("PLT_CODE")) _delparamTable2.Columns.Add("PLT_CODE", typeof(String)); //
                if (!_delparamTable2.Columns.Contains("FILE_ID")) _delparamTable2.Columns.Add("FILE_ID", typeof(String)); //
                if (!_delparamTable2.Columns.Contains("FILE_NAME")) _delparamTable2.Columns.Add("FILE_NAME", typeof(String));
                if (!_delparamTable2.Columns.Contains("REG_DATE")) _delparamTable2.Columns.Add("REG_DATE", typeof(String));
                if (!_delparamTable2.Columns.Contains("CONF_SECTION")) _delparamTable2.Columns.Add("CONF_SECTION", typeof(String));
                if (!_delparamTable2.Columns.Contains("CONF_NAME")) _delparamTable2.Columns.Add("CONF_NAME", typeof(String));
                ...
                if (selectedView.Count == 0)
                {
                    ...
                    if (!focusRow.Table.Columns.Contains("PLT_CODE")) { focusRow.Table.Columns.Add("PLT_CODE",typeof(String)); }

                    if (focusRow["PLT_CODE"].ToString() != "")
                    {
                    ...
                    {
                        foreach (DataRow row in _dtFileName.Select("FILE_NAME = '" + focusRow["FILE_NAME"].ToString() + "'"))
                        {
                            _dtFileName.Rows.Remove(row);
                        }

                        FileGridView.DeleteMappingRow(focusRow);
                ...
                else
                {
                    ...
                    for (int i = 0; i < cnt; i++)
                    {
                        if (!selectedView.Table.Columns.Contains("PLT_CODE")) { selectedView.Table.Columns.Add("PLT_CODE", typeof(String)); };

                        if (selectedView[i]["PLT_CODE"].ToString() != "")
                        {
                            DataRow paramRow = _delparamTable.NewRow();
                            paramRow["DEL_EMP"] = acInfo.UserID;
                            paramRow["PLT_CODE"] = acInfo.PLT_CODE;
                        ...
                        {
                            DataRow paramRow = _delparamTable2.NewRow();
                            paramRow["DEL_EMP"] = acInfo.UserID;
                            paramRow["PLT_CODE"] = acInfo.PLT_CODE;
                            paramRow["FILE_ID"] = selectedView[i]["FILE_ID"];
                            paramRow["FILE_NAME"] = selectedView[i]["FILE_NAME"];
                            ...
                            paramRow["CONF_SECTION"] = "SYS";
                            paramRow["CONF_NAME"] = "SYSTEM_GROUP";
                            _delparamTable2.Rows.Add(paramRow);

                        }
                        
                    }
                    foreach (DataRow row in _delparamTable2.Rows)
                    {
                        foreach (DataRow row2 in _dtFileName.Select("FILE_NAME = '" + row["FILE_NAME"].ToString() + "'"))
                        {
                            _dtFileName.Rows.Remove(row2);
                        }
                        FileGridView.DeleteMappingRow(row);
                    }
                }
                ...
                if (paramSet.Tables["RQSTDT"].Rows.Count != 0)
                {
            ...
            try
            {
                ...
                try
                {
                    ...
                    foreach (DataRow dr in _delparamTable.Rows)
                    {
                        ...
                        //string ftpDir = ftp.GetDirectory();
                        //string[] dirs = ftpDir.Split('/');
                        //string cur_dir = dirs[dirs.Length - 1];
                        //if (cur_dir != dir)
                        //{
                        //    if (cur_dir != "")
                        //    {
                        //        ftp.Invoke(FtpCommand.ChangeDirUp);
                        //        ftp.Invoke(FtpCommand.ChangeDir, dir);
                        //    }
                        //    else
                        //    {
                        //        ftp.Invoke(FtpCommand.ChangeDir, remotedir);
                        //    }
                        //}
