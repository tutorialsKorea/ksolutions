...
namespace MNT
{
    public sealed partial class MNT05A_M0A : BaseMenu
    {
        ...
        public override void ChildContainerInit(Control sender)
        {
            ...
            if (sender == acLayoutControl1)
            {
                ...
                //layout.GetEditor("S_DATE").Value = DateTime.Now.GetFirstDate();
                //layout.GetEditor("E_DATE").Value = DateTime.Now.GetFirstDate().AddDays(15);
        ...
        private int dateSwitch = 1;

        private void timer1_Tick(object sender, EventArgs e)
        {
            
            int LookupRows = (acLookupEdit1.Properties.DataSource as DataTable).Rows.Count;

            if (DateTime.Now.GetFirstDate() <= DateTime.Now && DateTime.Now.GetFirstDate().AddDays(14) >= DateTime.Now)
            {
                dateSwitch = 1; 
            }
            else
            ...
            rowIndex++;
            if (rowIndex == LookupRows) { rowIndex = 0; }
            acLookupEdit1.ItemIndex = rowIndex;

            if (rowIndex < LookupRows)
            {
                if (dateSwitch == 1)
                {
                    acLayoutControl1.GetEditor("S_DATE").Value = DateTime.Now.GetFirstDate();
                    acLayoutControl1.GetEditor("E_DATE").Value = DateTime.Now.GetFirstDate().AddDays(14);

                }
                else //(dateSwitch == 2) 
                {
                    acLayoutControl1.GetEditor("S_DATE").Value = DateTime.Now.GetFirstDate().AddDays(15);
                    acLayoutControl1.GetEditor("E_DATE").Value = DateTime.Now.GetLastDate();
                }

            }

            #region 이전 그룹별 월 상반기/하반기 조회하는 로직
            //if(rowIndex < LookupRows)  
            //{
            //    if(dateSwitch == 1)
            //    {
