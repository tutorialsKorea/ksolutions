...
namespace CodeHelperManager
{
    ...
    public class RepositoryItemEmp : RepositoryItemPopupContainerEdit
    {
        static RepositoryItemEmp()
        {
            Register();
        }


        public RepositoryItemEmp()
        {
            base.Buttons.Clear();
        ...
        internal const string EditorName = "acEmp";

        public static void Register()
        {

            EditorRegistrationInfo.Default.Editors.Add(new EditorClassInfo(EditorName, typeof(acEmp),
                typeof(RepositoryItemEmp), typeof(DevExpress.XtraEditors.ViewInfo.PopupContainerEditViewInfo),
    ...
    /// 사원 컨트롤
    /// </summary>
    public class acEmp : acUserPopupContainerEdit, ControlManager.IBaseEditControl
    {
        private object _AVAILMC = null;
        ...
        public acEmp()
        {
            ...
            if (ControlManager.acInfo.IsRunTime == true)
            {
                ...
                acEmp.SetPopupGridView(acGridView1);
            }
            ...
            this.CloseUp += new CloseUpEventHandler(acEmp_CloseUp);
            this.Popup += new EventHandler(acEmp_Popup);
            this.QuickFindEditor.KeyDown += new KeyEventHandler(QuickFindEditor_KeyDown);

            this.GotFocus += new EventHandler(acEmp_GotFocus);
            this.LostFocus += new EventHandler(acEmp_LostFocus);

        }

        void acEmp_LostFocus(object sender, EventArgs e)
        {
        ...
        void acEmp_GotFocus(object sender, EventArgs e)
        {
            if (acInfo.SysConfig.GetSysConfigByMemory("FOCUS_EDIT_ENABLED").toBoolean())
            {
        ...
        {
            //기본 컬럼
            view.AddTextEdit("EMP_CODE", "사원코드", "UV9LGK3D", true , DevExpress.Utils.HorzAlignment.Center, false, true, false, acGridView.emTextEditMask.NONE);
            view.AddTextEdit("EMP_NAME", "사원명", "40266", true , DevExpress.Utils.HorzAlignment.Center, false, true, false, acGridView.emTextEditMask.NONE);

            //옵션 컬럼
            view.AddLookUpEdit("EMP_TYPE", "사원형태", "U2V6VABY", true , DevExpress.Utils.HorzAlignment.Center, false, false, false, "S021");
            view.AddLookUpEdit("EMP_TITLE", "직책", "72MOO4VJ", true , DevExpress.Utils.HorzAlignment.Center, false, false, false, "C040");

            view.AddTextEdit("ORG_CODE", "부서코드", "40225", true , DevExpress.Utils.HorzAlignment.Center, false, false, false, acGridView.emTextEditMask.NONE);
            view.AddTextEdit("ORG_NAME", "부서명", "40223", true , DevExpress.Utils.HorzAlignment.Center, false, false, false, acGridView.emTextEditMask.NONE);
        ...
        void acGridView1_OnInitLayout(object sender)
        {
            ...
            view.LoadUserConfig("SYS04B_M0A", "acEmp", acInfo.DefaultConfigUser);
        ...
        void acEmp_CloseUp(object sender, CloseUpEventArgs e)
        {
            QuickFindEditor.EditValue = null;
        }

        void acEmp_Popup(object sender, EventArgs e)
        {
        ...
        public override string EditorTypeName
        {
            get { return RepositoryItemEmp.EditorName; }
        }



        [DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
        public new RepositoryItemEmp Properties
        {
            get
            {

                return base.Properties as RepositoryItemEmp;
        ...
        void Execute(emMethodType method, object parameter)
        {
            ...
            acEmpForm frmFind = new acEmpForm();
            ...
            if (frmFind.ShowDialog() == DialogResult.OK)
            {
                ...
                if (!this._Value.EqualsEx(result.Rows[0]["EMP_CODE"]))
                {
                ...
                this._Value = result.Rows[0]["EMP_CODE"];
        ...
        /// <summary>
        /// 현재컨트롤 설정으로인한 선택가능한 인원를 반환합니다.
        /// </summary>
        /// <returns></returns>
        public DataTable GetAvailEmp()
        {
            ...
            //DataSet resultSet = BizRun.QBizRun.ExecuteService(this, "CONTROL_EMP_SEARCH", paramSet, "RQSTDT", "RSLTDT");
            DataSet resultSet = BizRun.QBizRun.ExecuteService(this,"CTRL", "CONTROL_EMP_SEARCH", paramSet, "RQSTDT", "RSLTDT");
        ...
        /// <summary>
        /// 특정설비의 선택가능한 인원를 반환합니다.
        /// </summary>
        /// <returns></returns>
        public static DataTable GetAvailEmp(object mcCode)
        {
            ...
            //DataSet resultSet = BizRun.QBizRun.ExecuteService(GetClassName(), "CONTROL_EMP_SEARCH", paramSet, "RQSTDT", "RSLTDT");
            DataSet resultSet = BizRun.QBizRun.ExecuteService(GetClassName(),"CTRL", "CONTROL_EMP_SEARCH", paramSet, "RQSTDT", "RSLTDT");
        ...
        public static object GetCodeByName(object name)
        {
            ...
            paramTable.Columns.Add("EMP_NAME", typeof(String)); //부품코드

            DataRow paramRow = paramTable.NewRow();
            paramRow["PLT_CODE"] = acInfo.PLT_CODE;
            paramRow["EMP_NAME"] = name;
            paramTable.Rows.Add(paramRow);
            DataSet paramSet = new DataSet();
            paramSet.Tables.Add(paramTable);

            //DataTable data = BizRun.QBizRun.ExecuteService(GetClassName(), "CONTROL_EMP_SEARCH", paramSet, "RQSTDT", "RSLTDT").Tables[1];
            DataTable data = BizRun.QBizRun.ExecuteService(GetClassName(), "CTRL", "CONTROL_EMP_SEARCH", paramSet, "RQSTDT", "RSLTDT").Tables["RSLTDT"];

            if (data.Rows.Count != 0)
            {
                return data.Rows[0]["EMP_CODE"];
            }
...
//            DataTable data = BizRun.QBizRun.ExecuteService(GetClassName(), "CONTROL_EMP_SEARCH", paramSet, "RQSTDT", "RSLTDT").Tables[1];
        ...
        private bool SetCode(object code)
        {
            ...
            if (acChecker.isNull(code) == false)
            {
                ...
                //DataTable result = BizRun.QBizRun.ExecuteService(this, "CONTROL_EMP_SEARCH", paramSet, "RQSTDT", "RSLTDT").Tables[1];

                DataTable result = BizRun.QBizRun.ExecuteService(this, "CTRL", "CONTROL_EMP_SEARCH", paramSet, "RQSTDT", "RSLTDT").Tables["RSLTDT"];
