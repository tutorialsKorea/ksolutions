...
namespace POP
{
    public sealed partial class POP05B_M0A : BaseMenu
    {
        ...
        public override void MenuInit()
        {
            ...
            acGridView1.AddTextEdit("INS_SCOMMENT", "비고", "", false, DevExpress.Utils.HorzAlignment.Near, true, true, false, acGridView.emTextEditMask.NONE);
            ...
            acGridView1.AddHidden("IS_SHIP", typeof(byte));
            ...
            foreach (DataRow row in resultSet.Tables["RSLTDT"].Rows)
            {
                ...
                if (row["PROC_CODE"].ToString() == "P-10"
                    || row["PROC_CODE"].ToString() == "P-12")
                {
                    if (Conditions.Contains(row["PROC_CODE"].ToString()))
                    {
                        acCheckedComboBoxEdit2.AddItem(row["PROC_NAME"].ToString(), false, "", row["PROC_CODE"].ToString(), true, false, CheckState.Checked);
        ...
        private void AcGridView1_RowClick(object sender, RowClickEventArgs e)
        {
            ...
            if (hi.Column != null && hi.Column.FieldName == "ATTACH" && hi.InDataRow)
            {
                ...
                {
                    DataRow focusRow = acGridView1.GetFocusedDataRow();
                    POP05B_D1A frm = new POP05B_D1A(focusRow);
        ...
        private void btnLastEnd_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                if (selectedView.Count > 0)
                {
                    ...
                    {
                        if (selectedView[i]["IS_SHIP"].ToString() == "1")
                        {
                            isShip = true;
                            break;
                        }
                ...
                {
                    if (focusRow["IS_SHIP"].ToString() == "1")
                    {
                        isShip = true;
                    }
                }
                ...
                if (isShip)
                {
                    POP05B_D2A frm = new POP05B_D2A();
                    frm.ParentControl = this;
                    frm.DialogMode = BaseMenuDialog.emDialogMode.NEW;

                    if (frm.ShowDialog() == DialogResult.OK)
                    {
                        frmRow = (DataRow)frm.OutputData;
                    }
                    else
                    {
                        return;
                ...
                if (selectedView.Count > 0)
                {
                    for (int i = 0; i < selectedView.Count; i++)
                    {
                        ...
                        paramRow["IS_SHIP"] = shipFlag == "1" ? selectedView[i]["IS_SHIP"] : shipFlag;
                ...
                else
                {
                    if (focusRow["CHAIN_WO_NO"].ToString() == "")
                    {
                        ...
                        paramRow["IS_SHIP"] = shipFlag == "1" ? focusRow["IS_SHIP"] : shipFlag;
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < chainView.Count; i++)
                        {
                            ...
                            paramRow["IS_SHIP"] = shipFlag == "1" ? chainView[i]["IS_SHIP"] : shipFlag;
        ...
        private void btnNg_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                if (!base.ChildFormContains("NEW"))
                {
                    POP05B_D0A frm = new POP05B_D0A(acGridView1, null);
        ...
        private void acBarButtonItem4_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (acMessageBox.Show(this, "출하검사건에 대해 출하지시 하시겠습니까?", "", false, acMessageBox.emMessageBoxType.YESNO) != DialogResult.Yes)
            {
                return;
            }
            ...
            BizRun.QBizRun.ExecuteService(
            this, QBiz.emExecuteType.SAVE,
            "POP05A_UPD2", paramSet, "RQSTDT", "RSLTDT",
            QuickSaveShip,
            QuickException);
        }
        ...
        private void acBarButtonItem6_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (!base.ChildFormContains("NEW"))
            {
                ...
                POP05B_D0A frm = new POP05B_D0A(acGridView1, focusRow);
        ...
        private void acBarButtonItem7_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            foreach (DataRow row in modifyTable.Rows)
            {
                ...
                paramRow["WO_NO"] = row["WO_NO"];
                paramRow["SCOMMENT"] = row["INS_SCOMMENT"];
            ...
            BizRun.QBizRun.ExecuteService(
            this, QBiz.emExecuteType.SAVE,
            "POP05A_UPD4", paramSet, "RQSTDT", "RSLTDT",
            QuickSave2,
            QuickException);
        }
