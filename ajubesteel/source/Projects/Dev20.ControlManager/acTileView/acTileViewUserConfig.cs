...
namespace ControlManager
{
    ...
    [Serializable]
    public class acTileViewConfig : ISerializable
    {
        ...
        private acTileView _View = null;
        private acTileView _OriginView = null;


        public acTileViewConfig(acTileView view)
        {
            _View = view;
        ...
        public void Save(out byte[] layoutOutput, out byte[] configOutput)
        {
            ...
            foreach (acGridColumn col in _View.Columns)
            {
                ...
                acTileViewMask mask = new acTileViewMask(col.ColumnEdit);

                this._MaskDic.Add(col.FieldName, mask);

                /*
                if (col.EditorType == acTileView.emEditorType.LOOKUP || col.EditorType == acTileView.emEditorType.LOOKUP_CODE)
                {
        ...
        public void Load(object configName, object configMaker, byte[] layoutData, byte[] configData)
        {
            try
            {
                ...
                BinaryFormatter bformatter = new BinaryFormatter();
                bformatter.Binder = new acTileViewUserConfigSerializationBinder();
                ...
                acTileView systemGridView = new acTileView(systemGridControl);
                ...
                acTileViewConfig sysConfig = (acTileViewConfig)bformatter.Deserialize(sysConfigStrm);
                ...
                acTileViewConfig config = (acTileViewConfig)bformatter.Deserialize(loadConfigSt);
                ...
                //삭제될 컬럼 선정
                foreach (acTileViewColumn viewCol in _View.Columns)
                {
                    acTileViewColumn tempColumn = (acTileViewColumn)systemGridView.Columns[viewCol.FieldName];
                    acTileViewColumn viewColumn = (acTileViewColumn)_View.Columns[viewCol.FieldName];



                    if (viewCol.EditorType == acTileView.emEditorType.LOOKUP ||
                        viewCol.EditorType == acTileView.emEditorType.LOOKUP_CODE)
                    {
                ...
                foreach (acTileViewColumn tempCol in systemGridView.Columns)
                {
                    ...
                    if (!_View.Columns.Contains(_View.Columns[tempCol.FieldName]))
                    {
                        switch (tempCol.EditorType)
                        {
                            ...
                            case acTileView.emEditorType.CHECK:
                                ...
                                _View.AddCheckEdit(tempCol.FieldName, tempCol.Caption, tempCol.ResourceID, tempCol.UseResourceID, tempCol.OptionsColumn.AllowEdit, tempCol.Visible, (acTileView.emCheckEditDataType)tempCol.EditorData);

                                break;

                            case acTileView.emEditorType.TEXT:

                                _View.AddTextEdit(tempCol.FieldName, tempCol.Caption, tempCol.ResourceID, tempCol.UseResourceID, tempCol.AppearanceCell.TextOptions.HAlignment, tempCol.OptionsColumn.AllowEdit, tempCol.Visible, tempCol.IsRequired, (acTileView.emTextEditMask)tempCol.EditorData);
                            ...
                            case acTileView.emEditorType.DATE:
                                ...
                                _View.AddDateEdit(tempCol.FieldName, tempCol.Caption, tempCol.ResourceID, tempCol.UseResourceID, tempCol.AppearanceCell.TextOptions.HAlignment, tempCol.OptionsColumn.AllowEdit, tempCol.Visible, tempCol.IsRequired, (acTileView.emDateMask)tempCol.EditorData);
        ...
        private bool ConvertView(acTileView view, acTileViewConfig config)
        {
            try
            {
                foreach (acTileViewColumn col in view.Columns)
                {
                    ...
                    col.EditorType = (acTileView.emEditorType)config.EditorTypeDic[col.FieldName];

                    acTileViewMask mask = (acTileViewMask)config.MaskDic[col.FieldName];
        ...
        public acTileViewConfig(SerializationInfo info, StreamingContext context)
        {
        ...
        public void GetObjectData(SerializationInfo info, StreamingContext context)
        {
            ...
            info.AddValue("MaskDic", this._MaskDic, typeof(Dictionary<string, acTileViewMask>));
