...
namespace LogInForm
{
    public partial class MainForm_V4 : DevExpress.XtraEditors.XtraForm, IMainControl
    {
        ...
        public MainForm_V4(string serverIP,
            string databaseName,
            string plant,
            string lang,
            string userID,
        ...
        public MainForm_V4(string userID)
        {
            InitializeComponent();
            LoadMainForm_V3(userID);
        }
        ...
        {            
            ...
            if (acInfo.MenuLocation.isNullOrEmpty() || acInfo.MenuLocation.Equals("V") || acInfo.MenuLocation.Equals("V2"))
            {
                ...
                //메뉴리스트 추가
                this.LoadMenuGroupList(resultSet.Tables["MENU_LIST"]);
                //this.LoadMenuList(resultSet.Tables["MENU_LIST"]);
                ...
                navMenu.LinkSelectionMode = LinkSelectionModeType.OneInGroupAndAllowAutoSelect;
                //navMenu.NavigationPaneGroupClientHeight = 100;

                navMenu.PaintStyleKind = NavBarViewKind.Default;
                //navMenu.PaintStyleName = "SkinExplorerBarView";
                navMenu.PaintStyleName = "SkinNavigationPane";
                navMenu.ExplorerBarShowGroupButtons = false;
                ...
                //즐겨찾기 메뉴 expand
                foreach (KeyValuePair<string, DataRow> menu in _MyMenu)
                {
                    AddMyMenu(menu.Value);
                }
            ...
            try
            {
                ...
                if (hi != null && hi.InGroupCaption)
                {
                    if (!hi.InGroupButton)
                    {
                        ...
                        {
                            string curOpen = accordionControl1.Elements[0].Text;
                            if (hi.Group.Caption == curOpen)
                                return;
                        ...
                        {
                            AddMenu(acmenuGrp, this._MenuList, hi.Group.Hint);
                        }
                        
                        acmenuGrp.Expanded = true;
        ...
        private void LoadMenuGroupList(DataTable menuList)
        {

            NavBarGroup mymenu = new NavBarGroup("즐겨찾기");
            mymenu.LargeImage = Resource.feature_16x16;
            mymenu.Hint = "myMenu";
            
            //mymenu.GroupClientHeight = 500;

            this.navMenu.Groups.Add(mymenu);
            ...
            foreach (DataRow groupMenuRow in groupMenuRows)
            {
                ...
                grp.Hint = groupMenuCode;
                //grp.GroupClientHeight = 1000;
                ...
                if (!_OpenDocumentDic.ContainsKey(className))
                {
                    ...
                    if (loadControl is BaseMenu)
                    {
                        ...
                        if (menu.GetType() == typeof(WebView))
                        {
                            ...
                            //(menu as WebView).URL = menuRow["LINK_URL"].ToString();
                        }
