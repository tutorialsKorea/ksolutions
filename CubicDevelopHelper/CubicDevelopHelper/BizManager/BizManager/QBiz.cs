using System;
using System.Collections.Generic;
using System.Text;
using System.Windows.Forms;
using System.Threading;
using System.Data;
using System.IO;
using DevExpress.XtraEditors;
using DevExpress.XtraLayout;
using DevExpress.XtraTab;
using System.Data.SqlClient;
using System.Reflection;
using BizExecute;
using System.Net;
using System.Net.Json;
using System.Runtime.Serialization.Formatters.Binary;

namespace BizManager
{

    public sealed partial class QBiz
    {

        public class ExcuteCompleteArgs
        {
            public string className = null;

            public string ruleName = null;

            public DataSet result = null;

            public TimeSpan executeTime = TimeSpan.MinValue;

            public emExecuteType executeType = emExecuteType.NONE;

            public object parameter = null;


        }

        /// <summary>
        /// 
        /// </summary>
        /// <param name="sender">요청컨트롤</param>
        /// <param name="ruleName">Biz Rule</param>
        /// <param name="data">결과 데이터</param>
        /// <param name="executeTime">실행시간</param>
        public delegate void ExecuteCompleateInvoker(
            object sender,
            QBiz qBiz,
            ExcuteCompleteArgs e
            );



        /// <summary>
        /// 사용자 정의 예외발생
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="qBiz"></param>
        /// <param name="errorNumber"></param>
        public delegate void ExecuteExceptionInvoker(
            object sender,
            QBiz qBiz,
            BizException ex);



        /// <summary>
        /// 사용자 정의 예외발생
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="qBiz"></param>
        /// <param name="errorNumber"></param>
        public delegate void ExecuteStartInvoker(object sender, QBiz qBiz);


        //private ServerAgent.RemoteCall _RemoteCall = null;
        //private SqlConnection _DBConn = null;

        private string _ServerIP = string.Empty;
        private string _DBName = string.Empty;

        public enum emExecuteType
        {

            ///// <summary>
            ///// 로그인
            ///// </summary>
            //LOGIN,

            ///// <summary>
            ///// 로그아웃
            ///// </summary>
            //LOGOUT,

            /// <summary>
            /// 없음
            /// </summary>
            NONE,

            /// <summary>
            /// 확인
            /// </summary>
            CHECK,

            /// <summary>
            /// 읽기
            /// </summary>
            LOAD,

            /// <summary>
            /// 새로고침
            /// </summary>
            REFRESH,

            /// <summary>
            /// 상세 정보 읽기
            /// </summary>
            LOAD_DETAIL,

            /// <summary>
            /// 신규등록
            /// </summary>
            NEW,

            /// <summary>
            /// 저장
            /// </summary>
            SAVE,

            /// <summary>
            /// 삭제
            /// </summary>
            DEL,

            /// <summary>
            /// 처리
            /// </summary>
            PROCESS,

            /// <summary>
            /// 파라메터
            /// </summary>
            PARAMETER

        }

        public static string GetExecuteTypeString(QBiz.emExecuteType excuteType)
        {
            string typeMsg = string.Empty;

            switch (excuteType)
            {


                case QBiz.emExecuteType.LOAD:

                    //typeMsg = acInfo.Resource.GetString("읽기", "PKELTPXA");
                    typeMsg = "읽기";

                    break;

                case QBiz.emExecuteType.PROCESS:

                    //typeMsg = acInfo.Resource.GetString("처리", "39PXBV5E");
                    typeMsg = "처리";

                    break;

                case QBiz.emExecuteType.REFRESH:

                    //typeMsg = acInfo.Resource.GetString("갱신", "ELNMW11S");
                    typeMsg = "갱신";

                    break;
                case QBiz.emExecuteType.NEW:
                case QBiz.emExecuteType.SAVE:

                    //typeMsg = acInfo.Resource.GetString("저장", "7NKYXFU5");
                    typeMsg = "저장";

                    break;

            }

            return typeMsg;

        }

        public static string GetExecuteLogTypeString(QBiz.emExecuteType excuteType)
        {
            string typeMsg = string.Empty;

            switch (excuteType)
            {
                //case QBiz.emExecuteType.LOGIN:

                //    //typeMsg = acInfo.Resource.GetString("읽기", "PKELTPXA");
                //    typeMsg = "접속";

                //    break;

                //case QBiz.emExecuteType.LOGOUT:

                //    //typeMsg = acInfo.Resource.GetString("읽기", "PKELTPXA");
                //    typeMsg = "종료";

                //    break;

                case QBiz.emExecuteType.NONE:

                    //typeMsg = acInfo.Resource.GetString("읽기", "PKELTPXA");
                    typeMsg = "";

                    break;

                case QBiz.emExecuteType.CHECK:

                    //typeMsg = acInfo.Resource.GetString("읽기", "PKELTPXA");
                    typeMsg = "조회";

                    break;

                case QBiz.emExecuteType.PARAMETER:

                    //typeMsg = acInfo.Resource.GetString("읽기", "PKELTPXA");
                    typeMsg = "조회";

                    break;

                case QBiz.emExecuteType.LOAD:

                    //typeMsg = acInfo.Resource.GetString("읽기", "PKELTPXA");
                    typeMsg = "조회";

                    break;

                case QBiz.emExecuteType.LOAD_DETAIL:

                    //typeMsg = acInfo.Resource.GetString("읽기", "PKELTPXA");
                    typeMsg = "조회";

                    break;

                case QBiz.emExecuteType.PROCESS:

                    //typeMsg = acInfo.Resource.GetString("처리", "39PXBV5E");
                    typeMsg = "수정";

                    break;

                case QBiz.emExecuteType.REFRESH:

                    //typeMsg = acInfo.Resource.GetString("갱신", "ELNMW11S");
                    typeMsg = "조회";

                    break;

                case QBiz.emExecuteType.SAVE:

                    //typeMsg = acInfo.Resource.GetString("저장", "7NKYXFU5");
                    typeMsg = "수정";

                    break;

                case QBiz.emExecuteType.NEW:

                    //typeMsg = acInfo.Resource.GetString("저장", "7NKYXFU5");
                    typeMsg = "등록";

                    break;
                case QBiz.emExecuteType.DEL:

                    //typeMsg = acInfo.Resource.GetString("저장", "7NKYXFU5");
                    typeMsg = "삭제";

                    break;
            }

            return typeMsg;

        }

        //public QBiz(ServerAgent.RemoteCall rc)
        public QBiz()//(SqlConnection DBConn)
        {
            //_DBConn = DBConn;

        }

        private object _Parameter = null;

        public object Parameter
        {
            get { return _Parameter; }
            set { _Parameter = value; }
        }


        private Control _ParentControl = null;

        public Control ParentControl
        {
            get { return _ParentControl; }
            set { _ParentControl = value; }
        }

        private emExecuteType _ExecuteType = emExecuteType.LOAD;

        public emExecuteType ExecuteType
        {
            get { return _ExecuteType; }
            set { _ExecuteType = value; }
        }

        private string _ClassName = null;

        public string className
        {
            get { return _ClassName; }
            set { _ClassName = value; }
        }

        private string _RuleName = null;

        public string RuleName
        {
            get { return _RuleName; }
            set { _RuleName = value; }
        }

        private DataSet _RefData = null;

        public DataSet RefData
        {
            get { return _RefData; }
            set { _RefData = value; }
        }

        private string _InputTableName = null;

        public string InputTableName
        {
            get { return _InputTableName; }
            set { _InputTableName = value; }
        }

        private string _OutputTableName = null;

        public string OutputTableName
        {
            get { return _OutputTableName; }
            set { _OutputTableName = value; }
        }



        private ExecuteCompleateInvoker _CompleteCallBack = null;

        public ExecuteCompleateInvoker CompleteCallBack
        {
            get { return _CompleteCallBack; }
            set { _CompleteCallBack = value; }
        }

        private ExecuteExceptionInvoker _ExecuteExceptionCallBack = null;

        public ExecuteExceptionInvoker ExcuteExceptionCallBack
        {
            get { return _ExecuteExceptionCallBack; }
            set { _ExecuteExceptionCallBack = value; }
        }







        public QBiz(string serverip, string dbname)
        {
            this._ServerIP = serverip;
            this._DBName = dbname;
        }



        private QProgress _QBizProgress = null;


        private Control _FocusControl = null;


        private bool _IsThreadAbort = false;



        private bool _UseThreadPool = false;

        /// <summary>
        /// 쓰레드풀 사용여부
        /// </summary>
        public bool UseThreadPool
        {
            get { return _UseThreadPool; }
            set { _UseThreadPool = value; }
        }



        /// <summary>
        /// 서비스를 실행합니다.
        /// </summary>
        /// <param name="parentControl">호출하는 컨트롤</param>
        /// <param name="executeType">실행타입</param>
        /// <param name="className">클래스이름</param>
        /// <param name="ruleName">서비스이름</param>
        /// <param name="refData">데이터셋</param>
        /// <param name="inputTableName">입력테이블명</param>
        /// <param name="outputTableName">결과테이블명</param>
        /// <param name="completeCallBack">완료후 콜백함수</param>
        public void Start()
        {



            if (this._UseThreadPool == true)
            {

                ThreadPool.QueueUserWorkItem(new WaitCallback(ThreadProcessing));


            }
            else
            {
                ParameterizedThreadStart paramThread = new ParameterizedThreadStart(ThreadProcessing);

                Thread searchThead = new Thread(paramThread);



                searchThead.Start(new object[] { });

            }

            //if (this.ParentControl is IBase)
            //{
            //    IBase b = this.ParentControl as IBase;

            //    b.IsProcessing = true;

            //}


            //진행바 표시여부
            bool progressVisible = false;


            //진행바 텍스트
            string progressCaption = null;

            //진행바 닫기여부
            bool progressCloseButton = false;




            switch (_ExecuteType)
            {

                case emExecuteType.NONE:

                    progressVisible = false;

                    break;

                case emExecuteType.CHECK:

                    progressVisible = true;

                    //progressCaption = acInfo.Resource.GetString("확인중...", "85A9SJHF");
                    progressCaption = "확인중...";

                    progressCloseButton = true;

                    break;



                case emExecuteType.LOAD:

                    progressVisible = true;

                    //progressCaption = acInfo.Resource.GetString("읽는중...", "0O0BPWD8");
                    progressCaption = "읽는중...";

                    progressCloseButton = true;

                    //현재 포커스를 찾는다.
                    //this.FindFocusControl(_ParentControl, ref _FocusControl);

                    //this.SetControlEnbled(_ParentControl, false);

                    break;


                case emExecuteType.LOAD_DETAIL:


                    progressVisible = true;

                    //progressCaption = acInfo.Resource.GetString("상세정보 읽는중...", "TFJJKJGQ");
                    progressCaption = "상세정보 읽는중...";

                    progressCloseButton = true;

                    break;


                case emExecuteType.REFRESH:

                    progressVisible = true;

                    //progressCaption = acInfo.Resource.GetString("새로고침중...", "OBEGGOQ8");
                    progressCaption = "새로고침중...";

                    progressCloseButton = true;

                    //현재 포커스를 찾는다.
                    this.FindFocusControl(_ParentControl, ref _FocusControl);

                    //this.SetControlEnbled(_ParentControl, false);

                    break;




                case emExecuteType.NEW:
                case emExecuteType.SAVE:

                    progressVisible = true;

                    //progressCaption = acInfo.Resource.GetString("저장중...", "11PE5VS5");
                    progressCaption = "저장중...";

                    progressCloseButton = false;

                    //현재 포커스를 찾는다.
                    this.FindFocusControl(_ParentControl, ref _FocusControl);

                    //this.SetControlEnbled(_ParentControl, false);

                    break;

                case emExecuteType.DEL:

                    progressVisible = true;

                    //progressCaption = acInfo.Resource.GetString("삭제중...", "P8O6W3AI");
                    progressCaption = "삭제중...";

                    progressCloseButton = false;

                    //현재 포커스를 찾는다.
                    this.FindFocusControl(_ParentControl, ref _FocusControl);

                    //this.SetControlEnbled(_ParentControl, false);

                    break;


                case emExecuteType.PROCESS:

                    progressVisible = true;

                    //progressCaption = acInfo.Resource.GetString("처리중...", "RW353VO6");
                    progressCaption = "처리중...";

                    progressCloseButton = false;

                    //현재 포커스를 찾는다.
                    this.FindFocusControl(_ParentControl, ref _FocusControl);


                    //this.SetControlEnbled(_ParentControl, false);

                    break;

                case emExecuteType.PARAMETER:

                    progressVisible = true;

                    //progressCaption = acInfo.Resource.GetString("매개변수 생성중...", "LU09Y20I");
                    progressCaption = "매개변수 생성중...";

                    progressCloseButton = true;

                    //현재 포커스를 찾는다.
                    this.FindFocusControl(_ParentControl, ref _FocusControl);

                    //this.SetControlEnbled(_ParentControl, false);


                    break;



            }


            if (progressVisible == true)
            {
                //진행바 표시

                _QBizProgress = new QProgress(progressCaption, progressCloseButton);

                _QBizProgress.OnClose += new QProgress.CloseEventHandler(_QBizProgress_OnClose);

                if (_ParentControl.Parent != null)
                {
                    _ParentControl.Parent.Controls.Add(_QBizProgress);
                }
                else
                {
                    _ParentControl.Controls.Add(_QBizProgress);
                }





                int x = (_ParentControl.Width / 2) - (_QBizProgress.Width / 2);
                int y = (_ParentControl.Height / 2) - (_QBizProgress.Height / 2);

                _QBizProgress.Location = new System.Drawing.Point(x, y);

                _QBizProgress.BringToFront();



            }
            else
            {
                //if (this.ParentControl is BizIBase)
                //{
                //    BizIBase b = this.ParentControl as BizIBase;

                //    b.IsProcessing = false;


                //}                
            }



        }

        /// <summary>
        /// 현재 포커스를 가진 컨트롤을 찾는다.
        /// </summary>
        /// <param name="rootControl"></param>
        /// <param name="focusControl"></param>
        private void FindFocusControl(Control rootControl, ref Control focusControl)
        {
            foreach (Control child in rootControl.Controls)
            {
                if (child.Controls.Count != 0)
                {
                    FindFocusControl(child, ref focusControl);
                }

                if (child.Focused == true)
                {
                    focusControl = child;

                    return;
                }
            }

        }



        /// <summary>
        /// 컨트롤을 비활성화 한다.
        /// </summary>
        /// <param name="rootControl"></param>
        /// <param name="enbled"></param>
        private void SetControlEnbled(Control rootControl, bool enbled)
        {
            try
            {
                if (rootControl.Parent != null)
                {


                    foreach (Control child in rootControl.Parent.Controls)
                    {
                        child.Enabled = enbled;



                    }


                }
                else
                {
                    foreach (Control child in rootControl.Controls)
                    {
                        child.Enabled = enbled;


                    }


                }
            }
            catch
            {

            }


        }


        void _QBizProgress_OnClose()
        {
            this._IsThreadAbort = true;

            if (this._ParentControl.IsHandleCreated == true)
            {
                this._ParentControl.BeginInvoke(new MethodInvoker(ProgressClose));
            }

        }


        public void Abort()
        {
            this._IsThreadAbort = true;

            if (this._ParentControl.IsHandleCreated == true)
            {
                this._ParentControl.BeginInvoke(new MethodInvoker(ProgressClose));
            }

        }

        private void ThreadProcessing(object args)
        {
#if DEBUG

            BizExecute.BizExecute bizExcute = null;

            try
            {

                DateTime startTime = DateTime.Now;

                string strAssembly = "CUBIZ_BR";// +strPram[0].ToString();

                Assembly bizManager = Assembly.Load(strAssembly);

                Type assemType = null;

                try
                {
                    assemType = bizManager.GetType("B" + this._ClassName.Substring(0, 3) + "." + this._ClassName, true, true);
                }
                catch
                {
                    assemType = bizManager.GetType(this._ClassName + "." + this._ClassName, true, true);
                }

                //BizExecuteRun.BizConn = new BizExecute.BizExecute(this._ServerIP, this._DBName);
                bizExcute = new BizExecute.BizExecute(this._ServerIP, this._DBName);

                object obj = Activator.CreateInstance(assemType);

                MethodInfo mi = assemType.GetMethod(this._RuleName);

                //사업장코드 자동 삽입
                foreach (DataTable dt in this._RefData.Tables)
                {
                    if (!dt.Columns.Contains("PLT_CODE"))
                    {
                        dt.Columns.Add("PLT_CODE", typeof(string));
                        DataRow row = dt.NewRow();
                        row["PLT_CODE"] = ConnInfo.PLT_CODE;
                    }
                }

                object result = mi.Invoke(obj, new object[] { this._RefData, bizExcute });

                DataSet resultSet = (DataSet)result;
                //BizService.BizServiceSoap biz = new BizService.BizServiceSoap();

                //DataSet resultSet = biz.getBizr(this._RuleName,this._RefData,this._InputTableName, this._OutputTableName);

                #region execute log
                try
                {
                    Type assemLog = bizManager.GetType("CTRL.CTRL", true, true);
                    object logObj = Activator.CreateInstance(assemLog);
                    MethodInfo miLog = assemLog.GetMethod("CTRL_EXECUTE_LOG");
                    DataTable dtLog = new DataTable("RQSTDT");
                    dtLog.Columns.Add("useSe", typeof(string));
                    dtLog.Columns.Add("dataUsgqty", typeof(int));
                    dtLog.Columns.Add("ruleName", typeof(string));
                    DataRow logRow = dtLog.NewRow();
                    logRow["useSe"] = GetExecuteLogTypeString(this._ExecuteType);
                    using (Stream s = new MemoryStream())
                    {
                        BinaryFormatter formatter = new BinaryFormatter();
                        formatter.Serialize(s, this._RefData);
                        logRow["dataUsgqty"] = s.Length;
                    }
                    logRow["ruleName"] = this._RuleName;
                    dtLog.Rows.Add(logRow);
                    DataSet logData = new DataSet();
                    logData.Tables.Add(dtLog);
                    object logResult = miLog.Invoke(logObj, new object[] { logData, bizExcute });
                }
                catch { }

                #endregion

                DateTime endTime = DateTime.Now;

                TimeSpan executeTime = endTime.Subtract(startTime);

                if (this._IsThreadAbort == false)
                {

                    if (this._ExecuteType != emExecuteType.NONE)
                    {
                        if (this._ParentControl.IsHandleCreated == true)
                        {
                            this._ParentControl.BeginInvoke(new MethodInvoker(ProgressClose));
                        }
                    }


                    ExcuteCompleteArgs e = new ExcuteCompleteArgs();

                    e.className = this._ClassName;
                    e.ruleName = this._RuleName;
                    e.result = resultSet;
                    e.executeTime = executeTime;
                    e.executeType = this._ExecuteType;
                    e.parameter = this._Parameter;

                    if (this._CompleteCallBack != null)
                    {
                        if (this._ParentControl.IsHandleCreated == true)
                        {
                            this._ParentControl.BeginInvoke(this._CompleteCallBack, this._ParentControl, this, e);
                        }

                    }
                }


                bizExcute.commitClose();
                //ProgressClose();

            }
            catch (Exception ex)
            {

                //쓰레드 종료가 아니면 예외를 처리한다.
                if (bizExcute != null) bizExcute.rollbackClose();

                if (this._IsThreadAbort == false)
                {

                    if (this._ExecuteType != emExecuteType.NONE)
                    {
                        if (this._ParentControl.IsHandleCreated == true)
                        {
                            this._ParentControl.BeginInvoke(new MethodInvoker(ProgressClose));
                        }

                    }

                    if (ex.InnerException != null) ex = ex.InnerException;

                    BizException bizEx = new BizException(ex.Message, ex);

                    bizEx.RefDataIdx = 0;


                    if (ex.Data.Contains("LOC"))
                    {
                        bizEx.Data.Add("LOC", ex.Data["LOC"]);
                    }

                    if (ex.Data.Contains("DATA"))
                    {
                        bizEx.Data.Add("DATA", ex.Data["DATA"]);
                    }

                    if (ex.Data.Contains("ROW"))
                    {
                        bizEx.Data.Add("ROW", ex.Data["ROW"]);
                    }

                    string message = bizEx.Message.Replace(" ", string.Empty);


                    if (bizEx.Data.Contains("DATA"))
                    {


                        if (ExtensionMethod.isNumeric(bizEx.Data["DATA"].ToString()))
                        {
                            //Biz 사용자정의 에러 생성

                            bizEx.ErrNumber = ex.Data["DATA"].toInt32();

                            int start = message.IndexOf('[');

                            int end = message.IndexOf(']');

                            if (start >= 0 || end >= 0)
                            {

                                string parameterString = message.Substring(start + 1, (end - start) - 1);

                                string[] parameters = parameterString.Split(';');


                                foreach (string parameter in parameters)
                                {
                                    string[] ps = parameter.Split('=');

                                    bizEx.ParameterDic.Add(ps[0], ps[1]);

                                }
                            }


                            //if (bizEx.ParameterDic.Count != 0)
                            if (bizEx.Data.Contains("ROW"))
                            {
                                bizEx.ParameterData = new DataTable();

                                Dictionary<string, object> dics = (Dictionary<string, object>)(bizEx.Data["ROW"]);

                                foreach (KeyValuePair<string, object> dic in dics)
                                {
                                    switch (dic.Value.GetType().ToString())
                                    {
                                        case "System.String":
                                            bizEx.ParameterData.Columns.Add(dic.Key, typeof(string));
                                            break;
                                        case "System.DateTime":
                                            bizEx.ParameterData.Columns.Add(dic.Key, typeof(DateTime));
                                            break;
                                        case "System.Int32":
                                            bizEx.ParameterData.Columns.Add(dic.Key, typeof(int));
                                            break;
                                        case "System.Decimal":
                                            bizEx.ParameterData.Columns.Add(dic.Key, typeof(decimal));
                                            break;
                                        case "System.Byte":
                                            bizEx.ParameterData.Columns.Add(dic.Key, typeof(byte));
                                            break;
                                        default:
                                            bizEx.ParameterData.Columns.Add(dic.Key);
                                            break;
                                    }
                                    //bizEx.ParameterData.Columns.Add(dic.Key,dic.Value.GetType());

                                }
                                //foreach(DataColumn col in ((DataRow)bizEx.Data["ROW"]).Table.Columns)
                                //{
                                //    bizEx.ParameterData.Columns.Add(col.ColumnName);
                                //}

                                DataRow paramRow = bizEx.ParameterData.NewRow();

                                foreach (DataColumn col in bizEx.ParameterData.Columns)
                                {
                                    paramRow[col.ColumnName] = dics[col.ColumnName];
                                }

                                bizEx.ParameterData.Rows.Add(paramRow);
                                //bizEx.ParameterData.Rows.Add((DataRow)bizEx.Data["ROW"]);
                            }

                            if (this._ParentControl.IsHandleCreated == true)
                            {
                                this._ParentControl.BeginInvoke(this._ExecuteExceptionCallBack, this._ParentControl, this, bizEx);

                            }

                        }
                        else
                        {
                            if (this._ParentControl.IsHandleCreated == true)
                            {
                                this._ParentControl.BeginInvoke(this._ExecuteExceptionCallBack, this._ParentControl, this, bizEx);

                            }
                        }
                    }
                    else
                    {
                        if (this._ParentControl.IsHandleCreated == true)
                        {
                            this._ParentControl.BeginInvoke(this._ExecuteExceptionCallBack, this._ParentControl, this, bizEx);

                        }
                    }


                }

            }
#else
	try
	{
		DateTime startTime = DateTime.Now;

		var httpWebRequest = (HttpWebRequest)WebRequest.Create(ConnInfo.ApiUrl);
		httpWebRequest.ContentType = "application/json";
		httpWebRequest.Method = "POST";

		using (var streamWriter = new StreamWriter(httpWebRequest.GetRequestStream()))
		{
			System.IO.StringWriter sw = new System.IO.StringWriter();
			this._RefData.WriteXml(sw, XmlWriteMode.WriteSchema);

			JsonObjectCollection jsonObj = new JsonObjectCollection();
            jsonObj.Add(new JsonStringValue("EXECUTE_TYPE", executeType));
			jsonObj.Add(new JsonStringValue("CLASS_NAME", this._ClassName));
			jsonObj.Add(new JsonStringValue("RULE_NAME", this._RuleName));
			jsonObj.Add(new JsonStringValue("USER_ID", ConnInfo.UserID));
			jsonObj.Add(new JsonStringValue("DATA", sw.ToString()));

			streamWriter.Write(jsonObj);
			streamWriter.Flush();
			streamWriter.Close();
		}

		var httpResponse = (HttpWebResponse)httpWebRequest.GetResponse();
		using (var streamReader = new StreamReader(httpResponse.GetResponseStream()))
		{
			string result = streamReader.ReadToEnd();

			DateTime endTime = DateTime.Now;

			TimeSpan executeTime = endTime.Subtract(startTime);

			if (this._IsThreadAbort == false)
			{

				if (this._ExecuteType != emExecuteType.NONE)
				{
					if (this._ParentControl.IsHandleCreated == true)
					{
						this._ParentControl.BeginInvoke(new MethodInvoker(ProgressClose));
					}
				}


				ExcuteCompleteArgs e = new ExcuteCompleteArgs();

				e.className = this._ClassName;
				e.ruleName = this._RuleName;

				if (result.isNullOrEmpty()) e.result = null;
				else
				{
					StringReader xmlSr = new StringReader(result);
					
					DataSet paramSet = new DataSet();
					paramSet.ReadXml(xmlSr, XmlReadMode.Auto);

					e.result = paramSet;
				}

				e.executeTime = executeTime;
				e.executeType = this._ExecuteType;
				e.parameter = this._Parameter;

				if (this._CompleteCallBack != null)
				{
					if (this._ParentControl.IsHandleCreated == true)
					{
						this._ParentControl.BeginInvoke(this._CompleteCallBack, this._ParentControl, this, e);
					}

				}
			}
		}
	}
	catch (Exception ex)
	{
		if (this._IsThreadAbort == false)
		{

			if (this._ExecuteType != emExecuteType.NONE)
			{
				if (this._ParentControl.IsHandleCreated == true)
				{
					this._ParentControl.BeginInvoke(new MethodInvoker(ProgressClose));
				}

			}

			if (ex.InnerException != null) ex = ex.InnerException;

			BizException bizEx = new BizException(ex.Message, ex);

			bizEx.RefDataIdx = 0;


			if (ex.Data.Contains("LOC"))
			{
				bizEx.Data.Add("LOC", ex.Data["LOC"]);
			}

			if (ex.Data.Contains("DATA"))
			{
				bizEx.Data.Add("DATA", ex.Data["DATA"]);
			}

			if (ex.Data.Contains("ROW"))
			{
				bizEx.Data.Add("ROW", ex.Data["ROW"]);
			}

			string message = bizEx.Message.Replace(" ", string.Empty);


			if (bizEx.Data.Contains("DATA"))
			{


				if (ExtensionMethod.isNumeric(bizEx.Data["DATA"].ToString()))
				{
					//Biz 사용자정의 에러 생성

					bizEx.ErrNumber = ex.Data["DATA"].toInt32();

					int start = message.IndexOf('[');

					int end = message.IndexOf(']');

					if (start >= 0 || end >= 0)
					{

						string parameterString = message.Substring(start + 1, (end - start) - 1);

						string[] parameters = parameterString.Split(';');


						foreach (string parameter in parameters)
						{
							string[] ps = parameter.Split('=');

							bizEx.ParameterDic.Add(ps[0], ps[1]);

						}
					}


					//if (bizEx.ParameterDic.Count != 0)
					if (bizEx.Data.Contains("ROW"))
					{
						bizEx.ParameterData = new DataTable();

						Dictionary<string, object> dics = (Dictionary<string, object>)(bizEx.Data["ROW"]);

						foreach (KeyValuePair<string, object> dic in dics)
						{
							switch (dic.Value.GetType().ToString())
							{
								case "System.String":
									bizEx.ParameterData.Columns.Add(dic.Key, typeof(string));
									break;
								case "System.DateTime":
									bizEx.ParameterData.Columns.Add(dic.Key, typeof(DateTime));
									break;
								case "System.Int32":
									bizEx.ParameterData.Columns.Add(dic.Key, typeof(int));
									break;
								case "System.Decimal":
									bizEx.ParameterData.Columns.Add(dic.Key, typeof(decimal));
									break;
								case "System.Byte":
									bizEx.ParameterData.Columns.Add(dic.Key, typeof(byte));
									break;
								default:
									bizEx.ParameterData.Columns.Add(dic.Key);
									break;
							}
							//bizEx.ParameterData.Columns.Add(dic.Key,dic.Value.GetType());

						}
						//foreach(DataColumn col in ((DataRow)bizEx.Data["ROW"]).Table.Columns)
						//{
						//    bizEx.ParameterData.Columns.Add(col.ColumnName);
						//}

						DataRow paramRow = bizEx.ParameterData.NewRow();

						foreach (DataColumn col in bizEx.ParameterData.Columns)
						{
							paramRow[col.ColumnName] = dics[col.ColumnName];
						}

						bizEx.ParameterData.Rows.Add(paramRow);
						//bizEx.ParameterData.Rows.Add((DataRow)bizEx.Data["ROW"]);
					}

					if (this._ParentControl.IsHandleCreated == true)
					{
						this._ParentControl.BeginInvoke(this._ExecuteExceptionCallBack, this._ParentControl, this, bizEx);

					}

				}
				else
				{
					if (this._ParentControl.IsHandleCreated == true)
					{
						this._ParentControl.BeginInvoke(this._ExecuteExceptionCallBack, this._ParentControl, this, bizEx);

					}
				}
			}
			else
			{
				if (this._ParentControl.IsHandleCreated == true)
				{
					this._ParentControl.BeginInvoke(this._ExecuteExceptionCallBack, this._ParentControl, this, bizEx);

				}
			}
		}
	}
#endif

        }

        private void ProgressClose()
        {

            if (_ParentControl.Parent != null)
            {
                _ParentControl.Parent.Controls.Remove(_QBizProgress);

            }
            else
            {
                _ParentControl.Controls.Remove(_QBizProgress);
            }



            if (_QBizProgress != null)
            {
                _QBizProgress.Dispose();
            }



            this.SetControlEnbled(_ParentControl, true);

            //if (_FocusControl != null)
            //{
            //    if (_FocusControl is IBaseEditControl || _FocusControl is DevExpress.XtraEditors.TextBoxMaskBox)
            //    //if (_FocusControl is DevExpress.XtraEditors.TextBoxMaskBox)
            //    {
            //        _FocusControl.Focus();
            //    }
            //    else
            //    {
            //        //Control ctrl = BaseMenu.GetBaseControl(_FocusControl);
            //        Control ctrl = _FocusControl;
            //        if (ctrl != null)
            //        {

            //            if (ctrl.Parent != null)
            //            {
            //                ctrl.Parent.Focus();
            //            }
            //            else
            //            {
            //                ctrl.Focus();
            //            }
            //        }
            //    }

            //}

            //if (this.ParentControl is IBase)
            //{
            //    IBase b = this.ParentControl as IBase;

            //    b.IsProcessing = false;

            //}



        }


    }

    //public interface BizIBase
    //{

    //    void BarCodeScanInput(string barCode);

    //    bool IsBarCodeScaning { get; set; }

    //    void ChildContainerInit(Control sender);




    //    string Caption { get; }

    //    bool IsProcessing { get; set; }

    //    string MenuCode { get; }


    //}
}
