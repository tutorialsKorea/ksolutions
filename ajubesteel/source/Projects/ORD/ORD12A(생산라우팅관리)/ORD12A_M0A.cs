...
namespace ORD
{
    public sealed partial class ORD12A_M0A : BaseMenu
    {
        ...
        public override void MenuInit()
        {
            ...
            acGridView1.AddTextEdit("OS_PROD_QTY", "수주수량", "", false, DevExpress.Utils.HorzAlignment.Far, false, true, false, acGridView.emTextEditMask.NUMERIC);
            acGridView1.AddTextEdit("OS_PART_QTY", "BOM 수량", "", false, DevExpress.Utils.HorzAlignment.Far, false, true, false, acGridView.emTextEditMask.QTY);
            acGridView1.AddTextEdit("OS_QTY", "외주수량", "", false, DevExpress.Utils.HorzAlignment.Far, false, true, false, acGridView.emTextEditMask.QTY);
            ...
            acGridView1.AddCheckEdit("IS_ORD", "공정외주처리", "", false, false, true, acGridView.emCheckEditDataType._STRING);
            acGridView1.AddTextEdit("OS_ORD_EMP", "공정외주처리자 코드", "", false, DevExpress.Utils.HorzAlignment.Center, false, true, false, acGridView.emTextEditMask.NONE);
            acGridView1.AddTextEdit("OS_ORD_EMP_NAME", "공정외주처리자", "", false, DevExpress.Utils.HorzAlignment.Center, false, false, false, acGridView.emTextEditMask.NONE);
            acGridView1.AddDateEdit("OS_ORD_DATE", "공정외주처리일", "", false, DevExpress.Utils.HorzAlignment.Center, false, true, false, acGridView.emDateMask.SHORT_DATE);
        ...
        private void AcGridView1_CustomDrawCell(object sender, DevExpress.XtraGrid.Views.Base.RowCellCustomDrawEventArgs e)
        {
            try
            {
                if (sender is acGridView view)
                {
                    ...
                    if (row["IS_ORD"].ToString() == "1")
                    {
                        e.Appearance.BackColor = Color.MistyRose;
                        e.Appearance.ForeColor = Color.Black;
                    }
                    //else if (row["PROD_STATE"].ToString() == "5")
                    //{
        ...
        void Search()
        {
            ...
            paramTable.Columns.Add("WO_PART", typeof(String)); //
            ...
            paramTable.Columns.Add("IS_ORD_OS", typeof(String)); //
        ...
        private void barItemOsCreate_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                if (selected.Length == 0)
                {
                    ...
                    if (focusRow["IS_ORD"].ToString() == "1")
                    {
                        acAlert.Show(this, "이미 처리된 품목입니다.", acAlertForm.enmType.Info);
                        return;
                    }
                ...
                else
                {
                    ...
                    foreach (DataRow row in selected)
                    {
                        if (row["IS_ORD"].ToString() == "1")
                        {
                            acAlert.Show(this, "이미 처리된 품목이 포함되어 있습니다.", acAlertForm.enmType.Info);
                            return;
                        }
        ...
        void QuickUpdate(object sender, QBiz qBiz, QBiz.ExcuteCompleteArgs e)
        {
            try
            {
                e.result.Tables["RQSTDT"].Columns.Add("IS_ORD", typeof(string));
                foreach (DataRow row in e.result.Tables["RQSTDT"].Rows)
                {
                    row["IS_ORD"] = "1";
