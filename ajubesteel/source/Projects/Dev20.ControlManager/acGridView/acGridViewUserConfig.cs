...
namespace ControlManager
{
    ...
    [Serializable]
    public class acGridViewConfig : ISerializable
    {
        ...
        private acGridViewApperance _EditCellStyle = new acGridViewApperance();

        /// <summary>
        /// 수정가능한 Cell 스타일
        /// </summary>
        public acGridViewApperance EditCellStyle
        {
        ...
        private acGridViewApperance _ModifiedRowStyle = new acGridViewApperance();

        /// <summary>
        /// 변경된 행 스타일
        /// </summary>
        public acGridViewApperance ModifiedRowStyle
        {
        ...
        private acGridView _View = null;
        private acGridView _OriginView = null;


        public acGridViewConfig(acGridView view)
        {
            _View = view;
        ...
        public void Save(out byte[] layoutOutput, out byte[] configOutput)
        {
            ...
            foreach (acGridColumn col in _View.Columns)
            {
                ...
                acGridViewMask mask = new acGridViewMask(col.ColumnEdit);
        ...
        public void Load(object configName, object configMaker, byte[] layoutData, byte[] configData)
        {
            try
            {
                ...
                BinaryFormatter bformatter = new BinaryFormatter();
                bformatter.Binder = new acGridViewUserConfigSerializationBinder();
                ...
                acGridView systemGridView = new acGridView(systemGridControl);
                ...
                acGridViewConfig sysConfig = (acGridViewConfig)bformatter.Deserialize(sysConfigStrm);
                ...
                acGridViewConfig config = (acGridViewConfig)bformatter.Deserialize(loadConfigSt);
                ...
                //삭제될 컬럼 선정
                foreach (acGridColumn viewCol in _View.Columns)
                {

                    acGridColumn tempColumn = (acGridColumn)systemGridView.Columns[viewCol.FieldName];
                    acGridColumn viewColumn = (acGridColumn)_View.Columns[viewCol.FieldName];



                    if (viewCol.EditorType == acGridView.emEditorType.LOOKUP ||
                        viewCol.EditorType == acGridView.emEditorType.LOOKUP_CODE)
                    {
                ...
                foreach (acGridColumn tempCol in systemGridView.Columns)
                {
                    ...
                    if (!_View.Columns.Contains(_View.Columns[tempCol.FieldName]))
                    {
                        switch (tempCol.EditorType)
                        {
                            ...
                            case acGridView.emEditorType.CHECK:
                                ...
                                _View.AddCheckEdit(tempCol.FieldName, tempCol.Caption, tempCol.ResourceID, tempCol.UseResourceID, tempCol.OptionsColumn.AllowEdit, tempCol.Visible, (acGridView.emCheckEditDataType)tempCol.EditorData);

                                break;

                            case acGridView.emEditorType.TEXT:

                                _View.AddTextEdit(tempCol.FieldName, tempCol.Caption, tempCol.ResourceID, tempCol.UseResourceID, tempCol.AppearanceCell.TextOptions.HAlignment, tempCol.OptionsColumn.AllowEdit, tempCol.Visible, tempCol.IsRequired, (acGridView.emTextEditMask)tempCol.EditorData);
                            ...
                            case acGridView.emEditorType.DATE:
                                ...
                                _View.AddDateEdit(tempCol.FieldName, tempCol.Caption, tempCol.ResourceID, tempCol.UseResourceID, tempCol.AppearanceCell.TextOptions.HAlignment, tempCol.OptionsColumn.AllowEdit, tempCol.Visible, tempCol.IsRequired, (acGridView.emDateMask)tempCol.EditorData);
                            ...
                            case acGridView.emEditorType.BUTTON:
                                if(_OriginView != null
                                    && _OriginView.Columns.ColumnByFieldName(tempCol.FieldName) is acGridColumn buttonCol)
                                {
                                    _View.Columns.Add(buttonCol);
                                }
                                break;
        ...
        private bool ConvertView(acGridView view, acGridViewConfig config)
        {
            try
            {
                foreach (acGridColumn col in view.Columns)
                {
                    ...
                    col.EditorType = (acGridView.emEditorType)config.EditorTypeDic[col.FieldName];

                    acGridViewMask mask = (acGridViewMask)config.MaskDic[col.FieldName];
        ...
        public acGridViewConfig(SerializationInfo info, StreamingContext context)
        {
            try
            {
                this._EditCellStyle = (acGridViewApperance)info.GetValue("EditCellStyle", typeof(acGridViewApperance));
            }
            catch { }

            try
            {
                this._ModifiedRowStyle = (acGridViewApperance)info.GetValue("ModifiedRowStyle", typeof(acGridViewApperance));
            }
            catch { }
        ...
        public void GetObjectData(SerializationInfo info, StreamingContext context)
        {
            ...
            info.AddValue("MaskDic", this._MaskDic, typeof(Dictionary<string, acGridViewMask>));
